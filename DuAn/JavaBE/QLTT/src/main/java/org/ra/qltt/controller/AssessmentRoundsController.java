...
public class AssessmentRoundsController {
    ...
    public ResponseEntity<?> getAR(@RequestParam(name = "phase"  , required = false) Long phase){
        ...
        return ResponseEntity.ok(ResponseWrapper.success(assessmentRoundResponseDTOList,"Lấy danh sách thành công" , HttpStatus.OK.value()));
    }
    ...
    public ResponseEntity<?> getARById(@PathVariable Long id){
        ...
        return ResponseEntity.ok(ResponseWrapper.success(assessmentRoundResponseDTO,"Lấy dữ liệu thành công" , HttpStatus.OK.value()));
    }
